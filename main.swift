...
//     }
//     for (char,count) in lib {
//         if count == maxCount && str.firstIndex(of: char)! < str.firstIndex(of: maxChar)! {
//             maxChar = char
//         }
//     }
//     print(maxChar)
...
// Using optional binding to safely unwrap the optional result of readLine().
// Checking if lib[char] is nil before trying to access its value using optional binding.
// Updating the value of lib[char] using count + 1 instead of lib[char]! += 1.
// Checking if count == maxCount and if the index of the current character is less than the index of the current maxChar before updating maxChar.
